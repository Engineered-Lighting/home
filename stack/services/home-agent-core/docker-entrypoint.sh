...
case "$role" in
  ...
  phase3-migrate-owner-person)
    ...
    ;;
  phase3-migrate-owner-partner-access)
    [ "$#" -eq 1 ] || {
      echo "phase3 owner-partner-access migration accepts no arguments" >&2
      exit 64
    }
    run_phase3_migration "$PHASE3_OWNER_PARTNER_ACCESS_REVISION"
    ;;
  phase3-migrate-owner-person-role)
    [ "$#" -eq 1 ] || {
      echo "phase3 owner-person-role migration accepts no arguments" >&2
      exit 64
    }
