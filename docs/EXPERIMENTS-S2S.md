...
## Phase 2.6 (2026-05-12): Rust moshi-backend proven viable on Blackwell

Built Kyutai's production Rust binary (`moshi-backend`) from the
`rust/` subdir of `kyutai-labs/moshi`. First attempt at
`CUDA_COMPUTE_CAP=120` segfaulted at model load (Candle 0.9.1 doesn't
ship native sm_120 kernels yet as of commit `6d14a61`, Apr 24, 2026).
Rebuild with `CUDA_COMPUTE_CAP=90` (Hopper PTX) succeeded: Blackwell
JIT-compiles the PTX at load. Binary boots in ~10 s, warms model in
~3 s, listens on `https://0.0.0.0:8898/ws` (self-signed TLS,
auto-generated by entrypoint). VRAM: ~17 GB (vs ~14.7 GB for PyTorch
Moshi; the slight bump is the Rust allocator's overhead).

**Status:** Image builds + binary runs on Blackwell. Integration into
the bridge deferred — would require either (a) backporting
`MoshiListenerBackend`'s `ResultQueue` + dynamic-schema features to
the legacy `MoshiBackend` class, or (b) adapting `MoshiListenerUpstream`
to speak the MT-byte + Opus WS protocol the Rust binary uses. Both are
~1-2 day code changes. The PyTorch backend is already fast enough
(117 ms tool-call, 26 ms median frame cadence) for current real-world
use, so the Rust speed win (~2-3× per Kyutai's published numbers) is
on hold until either felt latency complaints surface or we need to free
the 14.7 GB the Python Moshi listener holds.

**Files for resuming this work:**
- `services/moshi-listener-rust/Dockerfile.moshi-rust-blackwell`
- `services/moshi-listener-rust/entrypoint.py`
- Image: `home-ai-voice/moshi-listener-rust:blackwell` (built and tested)
