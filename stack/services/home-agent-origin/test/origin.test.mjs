...
test("request and route parsers reject generic, native, query-bearing, and ambiguous paths", () => {
  ...
  assert.equal(browserApiRouteAllowed("GET", new URL("http://x/api/agent/v1/operator-rollout/phase3-readiness")), false);
  assert.equal(browserApiRouteAllowed("POST", new URL("http://x/api/agent/v1/operator-rollout/phase3-readiness")), false);
  assert.equal(browserApiRouteAllowed("GET", new URL("http://x/api/agent/v1/operator-rollout/phase3-readiness?debug=1")), false);
...
test("binding workflow proxies only exact browser paths with same-origin writes", async () => {
  ...
  try {
    ...
    for (const [method, target, origin] of [
      ...
      ["GET", "/api/agent/v1/operator-rollout/phase3-readiness", undefined],
      ["POST", "/api/agent/v1/operator-rollout/phase3-readiness", PUBLIC_ORIGIN],
      ["GET", "/api/agent/v1/operator-rollout/phase3-readiness?debug=1", undefined],
