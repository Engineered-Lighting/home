...
function HouseholdCard({
  ...
  })), !error && (relationships || []).length === 0 && (people || []).length > 0 && React.createElement("p", null, "No confirmed relationships yet."), !error && onAddPerson && React.createElement(React.Fragment, null, React.createElement("h3", null, "Add someone"), React.createElement("p", null, "Adding someone records that your household knows them. It does not give them an account or any authority here."), React.createElement("label", {
    htmlFor: "agent-person-name"
...
function HomeAgentPanel() {
  ...
  useEffect(() => {
    ...
    if (listen) {
      Promise.resolve(listen("native-auth-changed", () => {
        if (!disposed) refresh();
      })).then(stop => {
  ...
  return React.createElement("main", {
  ...
  }, "HOME AGENT"), React.createElement("h1", {
    className: "agent-title"
  }, "Governed intelligence")), React.createElement("div", {
    className: "agent-actions"
  }, React.createElement("button", {
    className: "agent-home",
    onClick: async () => {
      try {
    ...
    value: teaching,
    onChange: event => setTeaching(event.target.value),
    rows: 3
  }), React.createElement("button", {
    disabled: !teaching.trim() || !snapshot?.latest_visit?.visit_id,
    onClick: propose
  }, "Create review transaction"), transaction && React.createElement(React.Fragment, null, React.createElement("dl", {
    className: "agent-grid"
  }, React.createElement("dt", null, "Transaction state"), React.createElement("dd", null, transaction.state), React.createElement("dt", null, "Locator resolution"), React.createElement("dd", null, transaction.locator?.resolution || "unavailable"), React.createElement("dt", null, "Retained locator"), React.createElement("dd", null, transaction.locator?.specific_locator_retention || "unavailable"), React.createElement("dt", null, "Locator radius"), React.createElement("dd", null, transaction.locator?.radius_m ? `${transaction.locator.radius_m} m` : "unavailable"), React.createElement("dt", null, "Resolved parents"), React.createElement("dd", null, transaction.resolved_parents?.map(person => person.display_name).join(", ") || "unresolved")), React.createElement("pre", null, JSON.stringify(transaction, null, 2))), transaction?.state === "needs_confirmation" && transaction?.locator?.resolution !== "specific" && React.createElement("p", {
  ...
  }, "Explain this place relationship"), " ", React.createElement("button", {
    onClick: () => queryPlace("presence")
  }, "Are my parents here?"), relationship && React.createElement("pre", null, JSON.stringify(relationship, null, 2)), presence && React.createElement("pre", null, JSON.stringify(presence, null, 2))), transaction?.state === "committed" && transaction?.fact_id && React.createElement("section", {
    className: "agent-card"
  }, React.createElement("h2", null, "Correct, retract, or forget this descriptor"), React.createElement("p", null, "Every operation first shows Core\u2019s exact invalidation and preservation scope. Parent facts, visits, the place, and its locator are not generic edit targets."), React.createElement("textarea", {
    className: "agent-textarea",
    ...
    onClick: () => previewLifecycle("correction")
  }, "Preview correction"), " ", React.createElement("button", {
    onClick: () => previewLifecycle("retraction")
  }, "Preview retraction"), " ", React.createElement("button", {
    onClick: () => previewLifecycle("forget")
  }, "Preview descriptor-only forgetting"), lifecycle && React.createElement("pre", null, JSON.stringify(lifecycle, null, 2)), lifecycle?.preview_digest && !lifecycle?.confirmed && React.createElement("button", {
    onClick: confirmLifecycle
  }, "Confirm ", lifecycle.operation))), error && phase === "ready" && React.createElement("p", {
    className: "agent-error"
  }, error));
}
ReactDOM.createRoot(document.getElementById("root")).render(React.createElement(HomeAgentPanel, null));
