...
class ExtendedOpenAIBaseLLMEntity(Entity):
    ...
    async def _async_handle_chat_log(
        ...
        # M4 (Addendum 27 Section 6 Milestone 4): wrap each tool spec
        # with strict json_schema mode when env var
        # EXTENDED_OPENAI_STRICT_TOOLS=on. _adjust_schema (defined above
        # for the structured_output API path) already does the right
        # thing — strict:true, additionalProperties:false, optional
        # props become required-with-null-type. We just need to mutate
        # the parameters dict in place. Env-var-gated so deploy is
        # zero-risk; if vLLM rejects strict shape on any spec, set the
        # env back to off and we're back to status quo.
        import os
        strict_tools = (
            os.environ.get("EXTENDED_OPENAI_STRICT_TOOLS", "").strip().lower() == "on"
        )
        tools: list[ChatCompletionToolParam] = []
        for func_spec in function_tools:
            spec = func_spec["spec"]
            if strict_tools and isinstance(spec.get("parameters"), dict):
                # Mutate a COPY — never alter the source DEFAULT_CONF
                # because the spec dict is shared across all turns.
                import copy
                spec = copy.deepcopy(spec)
                _adjust_schema(spec["parameters"])
                # _adjust_schema sets strict + additionalProperties on
                # the inner parameters; OpenAI's tool spec also reads
        ...
        for n_requests in range(MAX_TOOL_ITERATIONS):
            ...
            for tool_input in pending_tool_calls:
                ...
                # M0.4 + M4: capture per-tool latency + result shape
                # so the baseline aggregator can chart tool latency over
                # time and compute malformed-args rate. Times only the
                # dispatch itself; lookup + validation are O(1).
                import time as _time
                _t0 = _time.monotonic()
                _tool_err: str | None = None
                try:
                ...
                except Exception as _e:  # noqa: BLE001 — captured for log only
                    _tool_err = type(_e).__name__
                    raise
                finally:
                    _lat_ms = int((_time.monotonic() - _t0) * 1000)
                    # Best-effort log; never break the dispatch loop on
                    # logging failure. Lazy-import keeps the import graph
                    # clean for any caller that hasn't loaded routing.
                    try:
                        from .external_routing import (
                            LOG_KIND_TOOL_CALL, log_decision,
                        )
                        # conversation_id lives on chat_log (HA's ChatLog
                        # API), not on llm_context. M4 v1 captured null
                        # here; v2 fixes correlation so M5's drawer can
                        # join tool_call entries with their routing_decision.
                        _conv_id = getattr(chat_log, "conversation_id", None)
                        _entry = {
                            "kind": LOG_KIND_TOOL_CALL,
                            "conv_id": _conv_id,
                            "tool_name": tool_input.tool_name,
                            "latency_ms": _lat_ms,
                            "ok": _tool_err is None,
                        }
                        if _tool_err is not None:
                            _entry["error_kind"] = _tool_err
                        # log_decision is async; schedule without await
                        # so we don't add a turn-blocking I/O point.
                        self.hass.async_create_task(
                            log_decision(self.hass, _entry)
                        )
