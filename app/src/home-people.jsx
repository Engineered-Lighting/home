...
function HomePeopleOverlay({ open, onClose, endpoint, token, client = null, connection = null, sim, spatialMode = false }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      // ready:false now means BOTH the legacy store and the agent authority
      // are unavailable: the integration falls back to the authority itself and
      // returns ready:true with source "agent_authority" when it can. The
      // browser has no route to the agent API from this origin, so there is
      // nothing useful to attempt here.
      ...
      if (!publishCurrentScope()) return;
      // Present only when the household came from the agent authority; the
      // legacy payload carries no edge list, and an absent list must read as
      // "no edges to draw" rather than as stale ones from a previous read.
      setAgentEdges(payload.relationships || []);
