...
Each helper publishes its two root-only master files atomically and refuses to
overwrite a complete or partial set. Neither prints a secret. The operator
stages reviewed proposals; the independent committer can eventually call only
the database binding kernel. Keep the deployment in `record_only`;
provisioning either role does not authorize binding or advance rollout mode.

See `BINDING-COMMITTER-ROLE.md` before advancing beyond the dormant E5b
revision.
